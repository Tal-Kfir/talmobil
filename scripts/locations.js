        ...
        if (owner && Markers.length < 11) {
            ...
            map.on('click', function(e) {   			
				...
				var formContent = `<form style="text-align:center" action="${window.location.pathname}" method="post" enctype="multipart/form-data">` + 
								...
								`<input class="button-inner-input type" type="text" name="lat" value="${popLocation['lat']}" required/><br/>` +
								`<input class="button-inner-input type" type="text" name="lon" value="${popLocation['lng']}" required/><br/>` +
...
function deleteLocation(lat, lon) {
    axios.post(window.location.pathname.toString() +"/delete", {
        withCredentials: true,
        location_del: [lon,lat]
          })
          .then((res) => {
            alert("Success");
